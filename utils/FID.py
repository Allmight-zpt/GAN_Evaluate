...
def compute_statistics_of_path(path, model, batch_size, dims, device, IMAGE_EXTENSIONS, num_workers=1):
...
# 对外接口
def calculate_fid_given_paths(paths, batch_size, device, dims, IMAGE_EXTENSIONS, num_workers=1):
    for p in paths:
    ...
    m1, s1 = compute_statistics_of_path(paths[0], model, batch_size, dims, device, IMAGE_EXTENSIONS, num_workers)
    m2, s2 = compute_statistics_of_path(paths[1], model, batch_size, dims, device, IMAGE_EXTENSIONS, num_workers)
